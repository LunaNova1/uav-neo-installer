...
def update(drone):
    ...
    _err_int += err * dt
    integral = uav_utils.clamp(_err_int*KI, -INT_CLAMP, INT_CLAMP)
    der_err = KD * (err - _prev_err)/dt
    pid = pid_control(err, integral, der_err, KP, KI, KD)

    throttle = uav_utils.clamp(pid + KFF * r_dot, - THROTTLE_LIMIT, THROTTLE_LIMIT)
    if _max_err <= abs(err):
        _max_err = abs(err)
    _prev_err = err

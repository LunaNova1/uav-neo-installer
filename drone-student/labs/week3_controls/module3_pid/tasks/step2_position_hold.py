...
def update(drone):
    ...
    _t += dt
    _pos += drone.physics.get_linear_velocity()[0] * dt
    ...
    _prev_err = err
    height_err = TARGET_HEIGHT - neo_lab.height(drone)
    ...
    speed = abs(drone.physics.get_linear_velocity()[0])
    ...
    if _t >= MIN_TRAVEL and _hold >= HOLD_TIME:
        print("Arrived")
        _done = True

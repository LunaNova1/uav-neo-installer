...
def update(drone):
    ...
    if gate is None:
        ...
        err_yaw = gate.cx - COL_CENTER
        drone.flight.send_pcmd(0, 0, uav_utils.clamp(err_yaw, -MAX_YAW, MAX_YAW), 0)
        print(f"Gate spotted")
        if abs(err_yaw) <= CENTER_TOL:
            err_pitch = gate.tag_px - TARGET_TAG_PX
            print(f"Pitching")
            drone.flight.send_pcmd(uav_utils.clamp(err_pitch, APPROACH_PITCH, APPROACH_PITCH), 0, 0, 0)
            if abs(err_pitch) <= CENTER_TOL:
                print(f"Done")
                _done = True
